    ...
    <div class="fire-status border-design">
      ...
      <div v-if="fireStatus.length > 0">
        <ul v-for="(item, index) in fireStatus" :key="index">
          <li>
            <a
              @click="handleFilterClicked('fire_status', item)"
              class="link_style"
              href="#"
              >{{ item }}</a
            >
    ...
    <div class="fire-cause border-design">
      ...
      <div v-if="fireCause.length > 0">
        <ul v-for="item in fireCause" :key="item">
          <li>
            <a
              @click="handleFilterClicked('fire_cause', item)"
              href="#"
              class="link_style"
              >{{ item }}</a
            >
      ...
      <div class="geo-desc-inner">
        <div v-if="geoDesc.length > 0">
          <ul v-for="item in geoDesc" :key="item">
            <li>
              <a
                @click="handleFilterClicked('geographic_description', item)"
                href="#"
                class="link_style"
                >{{ item }}</a
              >
...
export default {
  ...
  methods: {
    fetchAllFilters() {
      axios
        .get("http://137.184.172.232:8082/api/wildfire/")
        .then((response) => {
    ...
    handleFilterClicked(query, value) {
      this.permLink = `http://137.184.172.232:8082/api/wildfire/search/?${query}=${value}`;
